...
def trigger_database_seed():
    """Triggers authentic Kaggle SAML dataset seeding into CognoDB Cloud."""
    success = load_saml_dataset(DEFAULT_KAGGLE_CSV)
    return {
        "status": "SUCCESS" if success else "STANDBY",
        "message": "Seeded Kaggle SAML Dataset into CognoDB Cloud!" if success else "CognoDB credentials pending in .env"
    }
...
def recalculate_risk_scores():
    ...
    WITH a,
         10 +
         (CASE WHEN launderingTx > 0 THEN 35 + min(30, launderingTx * 5) ELSE 0 END) +
         (CASE WHEN structuringCount >= 2 THEN 20 ELSE (CASE WHEN structuringCount = 1 THEN 10 ELSE 0 END) END) +
